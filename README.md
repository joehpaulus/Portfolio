...
4. Push to `main` (or run the workflow manually). The site will be at `https://<username>.github.io/` (when the repo is named `<username>.github.io`).

If you rename the repo to something other than `<username>.github.io`, set `base` in `vite.config.js` to `'/YourRepoName/'` so asset paths are correct.
